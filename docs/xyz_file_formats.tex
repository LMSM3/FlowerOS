...
% (microtype omitted: requires scalable fonts)
...
precision: six decimal places ($\Rightarrow$ resolution $\approx
0.1$\,pm).
...
Fields are pipe-delimited.  The energy key \texttt{E =} is
recognised by the reader for energy-tracking contexts (minimisation
traces, conformer scans).  Additional free-text notes are permitted
after the second pipe.
...
Adds per-atom scalar and vector properties after the $(x, y, z)$
fields.
...
The \texttt{properties} key in the comment line declares which
optional columns are present.  Column order is fixed; omission is
permitted only from the right (i.e.\ you cannot include velocity
without charge).
...
The charge field $q_i$ represents the effective partial charge used
in Coulomb interactions.  It is \emph{not} an oxidation state.  For
...
to restart an interrupted MD run without loss of trajectory
continuity.

\subsection{Header Block}

The header block appears \emph{before} the atom-count line and is
delimited by \texttt{CHECKPOINT} / \texttt{END\_CHECKPOINT} markers:
...
\begin{enumerate}
  \item The header is parsed and the integrator state is restored
        (step counter, time accumulator, timestep, thermostat target,
        RNG state).
  \item The atom block is read as a standard \texttt{.xyza} frame
        with all available property columns.
  \item Neighbour lists and bond tables are reconstructed from
        coordinates; they are \emph{never} serialised into the
        checkpoint.
  \item If a \texttt{box} line is present, periodic boundary
        conditions are activated with the specified box geometry.
\end{enumerate}

\subsection{Checkpoint vs.\ Trajectory}

A \texttt{.xyzc} file contains exactly one frame.  It is a snapshot,
not a trajectory.  For multi-frame output, use \texttt{.xyzf}
(§\ref{sec:xyzf}).
...
A multi-frame format: one or more standard \texttt{.xyz} or
\texttt{.xyza} blocks concatenated in a single file, separated only
by the next atom-count line.  Used for minimisation traces, NVT
...
Each frame in a \texttt{.xyzf} file must be independently parseable.
This means:

\begin{itemize}
  \item The atom count $N$ may vary between frames (e.g.\ if atoms
        are added or removed during a grand-canonical simulation).
  \item The \texttt{properties} declaration may differ between frames
        (e.g.\ early frames may lack forces if computed lazily).
  \item Readers must re-parse the header of each frame; caching the
        first frame's column layout and applying it globally is a bug.
\end{itemize}
...
When periodic boundary conditions (PBC) are active, the box geometry
is stored either in the \texttt{.xyzc} header (§\ref{sec:xyzc}) or
as special comment-line keys in any format.
...
\begin{itemize}
  \item Box vectors are in \AA, given as a flattened $3\times 3$
        matrix (row-major).  Orthorhombic cells use diagonal entries
        only; triclinic cells populate off-diagonal terms.
  \item \texttt{pbc} flags indicate which axes are periodic
        (\texttt{T}) or open (\texttt{F}).
  \item The format is compatible with the ASE Extended XYZ convention,
        enabling interoperability with the Atomic Simulation Environment.
\end{itemize}
...
For triclinic cells, the full matrix is used and $V = \det(\mathbf{L})$
remains the volume.

\subsection{Minimum Image Convention}

Under PBC, the engine applies the minimum-image convention for
pairwise distances:
\begin{equation}
  ...
  \label{eq:mic}
\end{equation}
where $L_\alpha$ is the box length along axis $\alpha$ and the
$\mathrm{round}$ function returns the nearest integer.
...
Bonds are derived data, not ground truth.  Serialising bonds into
the file format would create a maintenance liability: any coordinate
update would require bond-table synchronisation, and stale bond
tables are a silent-corruption vector.  The \S0 container philosophy
(§0.2.6 of the Identity--State Decomposition) formalises this:
caches never become truth.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8.5cm}}
    ...
    \file{include/io/xyz\_format.hpp}
      & Struct definitions, public API for \type{XYZReader},
        \type{XYZWriter}, \type{XYZAReader}, \type{XYZAWriter} \\
...
Readers follow a common pattern:
...
These ranges serve as first-pass sanity checks.  Values outside
these bounds in production output should trigger audit warnings.
...
The XYZ file formats serve as the serialisation layer for the
particle identity vector $\mathbf{I}_i$ defined in §0 of the
Formation Engine methodology.  The mapping is:
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llp{6.5cm}}
    ...
    \midrule
    Nuclear identity  & $Z_i$      & Element symbol on atom line
                                      (looked up to $Z$) \\
    Mass identity     & $A_i$      & Implicit via $Z_i$
                                      (isotopic override not in format) \\
    Charge participation & $Q_i$   & \texttt{charge} column in
                                      \texttt{.xyza} \\
    Structural role   & $\Sigma_i$ & Not serialised; inferred from
                                      bond topology at load \\
    Stability class   & $\Lambda_i$& Not serialised; computed
                                      post-load \\
...
The cell container $\mathcal{K}$ maps to the file-level structure:
$\mathcal{K}_{\text{id}}$ is encoded in the filename and comment-line
provenance; $\mathcal{K}_{\text{geom}}$ is the lattice matrix from
the \texttt{Lattice} key or \texttt{box} header line;
$\mathcal{K}_{\text{comp}}$ is reconstructed at load from element
composition and geometry.

The source-layer flag $F_K \in \{Z, A, C\}$ in the cell identity
directly corresponds to the \type{XYZFormat} enumeration in the
codebase, mapping \texttt{Z}$\to$\texttt{.xyz},
\texttt{A}$\to$\texttt{.xyza}, \texttt{C}$\to$\texttt{.xyzc}.
...
Readers enforce the following validation rules at parse time:

\begin{enumerate}
  \item \textbf{Atom count mismatch.}  If fewer than $N$ atom lines
        follow the header, the reader throws
        \type{XYZParseError::INCOMPLETE\_FRAME}.
  \item \textbf{Unknown element.}  An unrecognised element symbol
        triggers \type{XYZParseError::UNKNOWN\_ELEMENT}.  Two-letter
        symbols must match the periodic table exactly (case-sensitive).
  \item \textbf{Malformed coordinates.}  Non-numeric coordinate fields
        trigger \type{XYZParseError::BAD\_COORDINATE}.
  \item \textbf{Column count mismatch.}  If the declared
        \texttt{properties} string implies $k$ columns per atom but
        a line has fewer, the reader emits a warning and fills missing
        fields with zero (fail-soft for velocities and forces; fail-hard
        for coordinates).
  \item \textbf{Checkpoint integrity.}  A \texttt{.xyzc} file missing
        the \texttt{CHECKPOINT}/\texttt{END\_CHECKPOINT} block is
        rejected with \type{XYZCParseError::NO\_HEADER}.
\end{enumerate}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lp{8cm}}
    ...
    Document & Relevance \\
    \midrule
    \file{docs/FILE\_FORMATS.md}
      & Prose description of all I/O formats \\
    \file{section0\_identity\_state\_decomposition.pdf}
      & Formal particle/cell/world container definitions \\
    \file{section5\_integration.tex}
      & MD timestep conventions and integrator details \\
    \file{ALPHA\_MODEL\_BOOKLET.tex}
      & Alpha model reference (Drude charge coupling) \\
    \file{section\_fuzzy\_ball\_model.tex}
      & Coarse-grained bead visualisation consuming XYZ data \\
    \bottomrule
  \end{tabular}
\end{table}
